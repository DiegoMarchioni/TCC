...
\address{Instituto de Ciências Exatas e Informática\\
  ...
  \email{\{diego.marchioni, marco.rodrigo\}@sga.pucminas.br}
}
...
language runtime or syntax, the comparison is operationalized indirectly: for
each mechanism, we measure its distance from an imperative baseline written in
the same language, and then contrast the two distances. The work combines a
...
output (two effect mechanisms and two imperative baselines) and an experimental
protocol based on Criterion and Bechamel. This article reports the state of the
work at the first checkpoint of the final-year research project: the four
implementations, a semantics-derived oracle test suite, a deterministic
cross-language workload generator, and both benchmark harnesses exist as code,
with validation in the definitive environment under way. Counting lines of code
over the actual implementations revealed that this metric is highly sensitive
to formatting style and to the explicit sequencing OCaml requires, which led to
restricting code-metric comparisons to within-language pairs, mirroring the
normalization already adopted for performance. Empirical measurements are
pending and belong to the next stage.
\end{abstract}
...
mutável, exceções e saída (dois mecanismos de efeitos e duas linhas de base
imperativas) e um protocolo experimental baseado em Criterion e Bechamel. Este
artigo reporta o estado do trabalho no primeiro ponto de controle: as quatro
implementações, uma suíte de testes de oráculo derivada da semântica formal, um
gerador determinístico de cargas replicado nas duas linguagens e os dois
\textit{harnesses} de \textit{benchmark} existem como código, com a validação
no ambiente definitivo em curso. A contagem de linhas sobre as implementações
efetivas revelou que essa métrica é fortemente sensível ao estilo de formatação
e ao sequenciamento explícito exigido pelo OCaml, o que levou a restringir as
comparações de métricas de código a pares intralinguagem, espelhando a
normalização já adotada para o desempenho. As medições empíricas estão
pendentes e constituem a etapa seguinte.
\end{resumo}
...
verificação formal. A ela somam-se os mecanismos de composição que sustentam a
modularidade do paradigma \cite{hughes:89}; o conjunto entra, porém, em tensão
direta com a necessidade de ler arquivos, lançar exceções e manipular estado
mutável. Essas capacidades são
...
Para conciliar pureza referencial e expressividade prática, dois mecanismos
...
\cite{kiselyov:13,pretnar:15}. Embora a teoria seja definida algebricamente, a
...
de abstração e desempenho. Os estudos existentes tratam de formalização
\cite{plotkin:03,pretnar:15}, de desempenho isolado
\cite{sivaramakrishnan:21,leijen:17} ou de expressividade em linguagens
específicas \cite{lindley:17,xie:20}, mas não integram essas dimensões; a
Seção~\ref{sec:lacuna} detalha esse levantamento. O problema que este trabalho
aborda é, portanto, a ausência de evidência comparativa integrada e mensurável
entre mônadas e efeitos algébricos.
...
imperativa} na sua própria linguagem, e as duas razões resultantes é que são
confrontadas. Por linha de base imperativa entende-se uma implementação do
...
\begin{itemize}
  \item[\textbf{QP1}] Qual abordagem acrescenta menos cerimônia sintática de
        efeitos e menos profundidade de aninhamento --- indicadores estruturais
        associados à carga de leitura --- sobre a linha de base da sua própria
        linguagem? Caso os dois indicadores divirjam, a resposta é relatada por
        indicador, sem agregação.
...
está fora do escopo, sendo indicada como trabalho futuro
(Seção~\ref{sec:metricas}). As hipóteses associadas são enunciadas na
...
\begin{enumerate}
  \item Uma análise conceitual estruturada das diferenças entre mônadas e
        ...
        (Seção~\ref{sec:fundamentacao}, em especial~\ref{sec:relacao}).
...
\medskip
\noindent\textbf{Estado do trabalho.} Este artigo corresponde ao primeiro ponto
de controle do TCC~II \cite{marchioni:tcc1}. As contribuições 1, 2 e 4 estão
...
lacuna. A Seção~\ref{sec:metodologia} descreve a metodologia efetivamente
adotada. A Seção~\ref{sec:impl} descreve as implementações desenvolvidas. A
...
As duas apresentações da mônada são equivalentes. A formulação monoidal
...
A pilha \texttt{StateT~(ExceptT~IO)} é o exemplo canônico dessa dependência de
ordem e, precisamente por descartar o estado no erro, não é a adotada neste
...
\emph{macro-expressividade}: a pergunta não é se ambos computam as mesmas
funções, o que é trivialmente verdadeiro, mas se um pode ser traduzido no outro
por transformação local e composicional, sem reescrita global do programa. O
...
\emph{Terceiro}, os três efeitos deste caso de uso são algébricos e de uso
\emph{linear} da continuação. São algébricos porque cada um admite apresentação
por assinatura e equações com modelo livre:
\begin{itemize}
...
\end{itemize}
São de uso linear porque \texttt{get}, \texttt{set} e \texttt{out} retomam a
continuação exatamente uma vez e \texttt{raise} a descarta, de modo que nenhum
...
teoria admite apresentação \emph{algébrica} (de modelo livre) \cite{plotkin:09};
...
\subsection{Abordagens Alternativas de Composição}
...
\subsection{Implementação e Expressividade de Efeitos Algébricos}
...
Dois trabalhos são os mais diretamente relacionados. O primeiro descreve as
...
integrado. A Tabela~\ref{tab:lacuna} sintetiza a cobertura dos trabalhos
...
casamento de padrão e os blocos condicionais.

Essas métricas são \emph{proxies} de legibilidade, não medidas diretas da
legibilidade percebida; adotam-se múltiplos \emph{proxies} complementares. Para
...
\begin{lstlisting}[style=ocaml,caption={Declaração dos efeitos e avaliador em OCaml~5.},label={lst:ocaml-eval}]
type _ Effect.t +=
...
let rec eval : expr -> int = function
  ...
  | Let (x, e, b) ->
      ...
      perform (Set (x, v));
      eval b
  | Seq (a, b) -> ignore (eval a); eval b
  | Print e -> let v = eval e in perform (Output v); v
  (* Mul analogo a Add *)
\end{lstlisting}
...
efeitos (Listagem~\ref{lst:imperative}). A tabela é recebida como parâmetro, e
não capturada de um escopo global, para que o padrão de acesso seja idêntico ao
da versão com \textit{handlers}. O cotejo com a Listagem~\ref{lst:ocaml-eval}
isola o que o mecanismo de efeitos acrescenta: a estrutura do avaliador é a
mesma, e a diferença reduz-se à substituição de acessos diretos à tabela e de
\texttt{raise} por operações de efeito.

\begin{lstlisting}[style=ocaml,caption={Referência imperativa em OCaml. Mesmas mensagens canônicas e mesma fixação de ordem da versão com efeitos.},label={lst:imperative}]
let rec eval (env : (string, int) Hashtbl.t) : expr -> int = function
  | Num n -> n
  | Var x -> (
      match Hashtbl.find_opt env x with
      | Some v -> v
      | None -> raise (Runtime_error (msg_unbound x)))
  | Add (a, b) ->
      let x = eval env a in
      let y = eval env b in
      x + y
  | Div (a, b) ->
      let x = eval env a in
      let y = eval env b in
      if y = 0 then raise (Runtime_error msg_div_zero) else x / y
  | Let (x, e, b) ->
      let v = eval env e in
      Hashtbl.replace env x v;
      eval env b
  | Seq (a, b) -> ignore (eval env a); eval env b
  | Print e -> let v = eval env e in !emit v; v
\end{lstlisting}
...
lida pelas duas linguagens (Listagem~\ref{lst:oraculo}). São 25 casos por
implementação, cobrindo todos os construtores da Tabela~\ref{tab:syntax} em
situações de sucesso e de erro, incluindo estados iniciais não vazios e
operandos negativos.

\begin{lstlisting}[style=ocaml,caption={Excerto da suíte de oráculo. Cada caso fixa resultado, ambiente final e traço de saída.},label={lst:oraculo}]
(* Divisao truncada em direcao a zero, com sinais opostos:
   quot (-7) 2 = -3, ao passo que div (-7) 2 = -4 *)
("divisao truncada, negativo", Div (Num (-7), Num 2),
 { result = Ok (-3); env = []; out = [] });

(* Ordem de avaliacao dos operandos: fixada a esquerda *)
("ordem de avaliacao", Add (Print (Num 1), Print (Num 2)),
 { result = Ok 3; env = []; out = [ 1; 2 ] });

(* O vinculo de Let persiste apos o corpo: ambiente global *)
("let persiste apos o corpo", Seq (Let ("x", Num 1, Num 0), Var "x"),
 { result = Ok 1; env = [ ("x", 1) ]; out = [] });

(* Estado e saida sobrevivem ao erro *)
("estado sobrevive ao erro",
 Let ("x", Num 5, Seq (Print (Var "x"), Div (Num 1, Num 0))),
 { result = Error msg_div_zero; env = [ ("x", 5) ]; out = [ 5 ] });
\end{lstlisting}

Quatro grupos de casos merecem destaque por travarem decisões de projeto, e não
apenas por verificarem comportamento. O caso da divisão truncada usa operandos
de sinais opostos, única configuração em que \texttt{quot} e \texttt{div}
divergem, e portanto a única capaz de detectar a troca. O caso de ordem de
avaliação usa \texttt{Print} nos dois operandos, única configuração que torna a
ordem observável, e falha se o compilador avaliar o operando direito primeiro.
Os casos de persistência e revinculação de \texttt{Let} falham se alguma
implementação introduzir restauração de escopo. O caso em que estado e saída
sobrevivem ao erro falha se a pilha monádica for invertida, o que transforma uma
decisão sutil de ordenação de camadas em uma verificação automática.
...
\subsection{Atividades Desenvolvidas desde o TCC~I}

\begin{enumerate}
  \item \textbf{Revisão crítica do relatório do TCC~I} \cite{marchioni:tcc1}.
  A releitura do relatório identificou duas divergências em relação à semântica
  pretendida, corrigidas antes da implementação: a divergência entre
  \texttt{div} e \texttt{/} na divisão inteira e a ordem das camadas da pilha
  monádica, que descartava o estado no erro. A terminologia da semântica também
  foi corrigida: a regra \textbf{(Let)} implementa um ambiente global, e não
  escopo léxico --- correção de descrição, não de comportamento pretendido, mas
  hoje travada por caso de teste.
  \item \textbf{Formalização da semântica e do critério de equivalência.}
  Fixaram-se o domínio semântico, os contextos de avaliação, a definição de
  ortogonalidade e o critério de equivalência funcional com quatro cláusulas
  (Seção~\ref{sec:semantica}), que substituem a descrição informal do TCC~I.
  \item \textbf{Implementação das quatro versões.} Desenvolvidas as versões com
  efeitos algébricos, monádica e as duas linhas de base, além do núcleo comum de
  sintaxe e geração determinística de cargas (Seção~\ref{sec:impl}). O exercício
  de implementação expôs dois problemas que a especificação em papel não
  revelava: a ordem de avaliação não especificada dos argumentos em OCaml e o
  tratamento ausente de variável não vinculada na versão com efeitos. Ambos
  foram corrigidos e estão hoje cobertos por casos de teste.
  \item \textbf{Fixação do ferramental e do ambiente de medição.} Construção do
  \textit{switch} opam com \textit{flambda}, substituição da versão do OCaml
  planejada (5.1.1) pela 5.2.1 em razão da maturidade do suporte a Windows, e
  identificação de que a \textit{flag} \texttt{-O3} é silenciosamente ignorada
  sem \textit{flambda} (Seção~\ref{sec:organizacao}).
  \item \textbf{Construção da suíte de oráculo.} Implementada nas duas
  linguagens, com casos derivados da semântica formal, incluindo casos que
  travam decisões de projeto (Seção~\ref{sec:oraculo}), e acompanhada de um teste
  de conformidade entre as réplicas dos artefatos comuns.
  \item \textbf{Implementação dos \textit{harnesses}.} Os cenários de carga
  implementados com Criterion e Bechamel (Seção~\ref{sec:harness}).
  \item \textbf{Atualização da revisão de literatura.} Foram incorporados
  trabalhos posteriores ao levantamento inicial, com destaque para a
  caracterização formal da correspondência entre efeitos modulares e
  transformadores \cite{schrijvers:19}, para a comparação intralinguagem entre
  \textit{handlers} e transformadores \cite{xie:20} e para a linha de pesquisa
  sobre redução do sobrecusto de \textit{handlers}
  \cite{karachalias:21,gaissert:25}.
  \item \textbf{Refinamento da metodologia de métricas.} A contagem de LOC sobre
  o código efetivo motivou a restrição das comparações de código a pares
  intralinguagem e a reformulação de H1, H2 e H3 nesses termos
  (Seção~\ref{sec:refinamento}).
\end{enumerate}
...
\begin{enumerate}
  \item[\textbf{A1}] \textbf{Validação no ambiente definitivo.} Compilação das
  quatro versões e execução da suíte de oráculo e do teste de conformidade até a
  passagem integral, resolvendo as pendências da Seção~\ref{sec:estado}.
  \item[\textbf{A2}] \textbf{Publicação do repositório.} Disponibilização
  pública do código, do arquivo de conformidade, do documento de protocolo de
  métricas e das configurações dos formatadores, com depósito arquivado e
  identificador persistente.
  \item[\textbf{A3}] \textbf{Implementação do quarto efeito.} Inserção do efeito
  \texttt{Input} nas quatro implementações, em ramo separado, com registro dos
  pontos de modificação e das adições puras, para responder a QP2. Reexecução da
  suíte de oráculo sobre o ramo, conforme a pré-condição da
  Seção~\ref{sec:protocolo}.
  \item[\textbf{A4}] \textbf{Coleta de métricas de código.} Normalização por
  formatadores e contagem por dupla verificação independente do autor e do
  orientador, conforme o documento de protocolo.
  \item[\textbf{A5}] \textbf{Execução dos \textit{benchmarks}.} Execução dos
  quatro cenários nas condições da Seção~\ref{sec:protocolo}, sobre a versão de
  três efeitos, com 30 amostras por configuração, três sementes e intervalos de
  confiança por \textit{bootstrap}.
  \item[\textbf{A6}] \textbf{Redação de Resultados e Discussão.} Preenchimento
  das Tabelas~\ref{tab:metrics} e~\ref{tab:perf}, interpretação à luz de H1--H4
  e P1, e discussão dos cenários em que cada abordagem é recomendada.
  \item[\textbf{A7}] \textbf{Revisão final e submissão.} Revisão de
  consistência, atualização das Seções~\ref{sec:intro} a~\ref{sec:relacionados}
  à luz dos resultados, e reexecução em ambiente limpo para confirmar a
  \emph{repetibilidade} --- a \emph{reprodutibilidade} por terceiros é viabilizada
  pelos artefatos publicados em A2.
\end{enumerate}

%% TODO: ajustar as colunas do cronograma ao calendario vigente antes da entrega.
\begin{table}[ht]
  \centering
  \caption{Cronograma das próximas atividades, com dependências. Uma atividade
           só se inicia após a conclusão daquelas de que depende.}
  \label{tab:cronograma}
  \small\setlength{\tabcolsep}{4pt}
  \begin{tabular}{llccccc}
    \toprule
    \textbf{Atividade} & \textbf{Dep.} & \textbf{Set} & \textbf{Out} & \textbf{Nov} & \textbf{Dez} & \textbf{Jan} \\
    \midrule
    A1. Validação no ambiente definitivo   & ---     & $\bullet$ &           &           &           &           \\
    A2. Publicação do repositório          & A1      & $\bullet$ &           &           &           &           \\
    A3. Implementação do quarto efeito     & A1      &           & $\bullet$ &           &           &           \\
    A4. Coleta de métricas de código       & A3      &           & $\bullet$ & $\bullet$ &           &           \\
    A5. Execução dos \textit{benchmarks}   & A1      &           &           & $\bullet$ &           &           \\
    A6. Redação de Resultados e Discussão  & A4,\,A5  &           &           & $\bullet$ & $\bullet$ &           \\
    A7. Revisão final e submissão          & A6      &           &           &           & $\bullet$ & $\bullet$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[ht]
  \centering
  \caption{Riscos identificados e respostas previstas.}
  \label{tab:riscos}
  \small
  \begin{tabular}{p{0.34\linewidth}p{0.58\linewidth}}
    \toprule
    \textbf{Risco} & \textbf{Resposta} \\
    \midrule
    O \textit{switch} com \textit{flambda} não é construído com êxito na
    plataforma escolhida &
    Medir sem \textit{flambda}, declarando a mudança de configuração; em último
    caso, migrar a medição para Linux, registrando a alteração de ambiente entre
    as ameaças à validade \\
    A API do Bechamel diverge entre versões &
    Versão fixada no \texttt{opam} e citada no texto \\
    Atraso em A1, que precede todas as demais &
    A2 e A3 podem ser antecipadas parcialmente; A5 dispõe de folga em dezembro \\
    \bottomrule
  \end{tabular}
\end{table}
